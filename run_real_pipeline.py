...
def main() -> None:
    ...
    print("\n" + "=" * 70)
    print("V30 ROOT-LEVEL FIXES VERIFIED IN THIS RUN:")
    print("=" * 70)
    print("  Compound #1 (10.25): validated_hypotheses.csv NOT merged into KNOWN_POSITIVES")
    print("                       +0.1 bonus SKIPPED for KP pairs (no circular leakage)")
    print("  Compound #2 (10.26/10.10): gnn_score weight capped at 0.20 (was 0.35)")
    print("                       z-score normalization replaces D3 weight-amplification no-op")
    print("  Compound #3 (3.9/3.10): W-02 multi-hop path injection REMOVED")
    print("                       random 'known positives' generation REMOVED")
    print("  Compound #4 (10.29): gamma=0.0 (contextual bandit, was 0.95)")
    print("  10.12: HIGH/LOW reward asymmetry fixed (bad_high_penalty_scale=0.05)")
    print("  10.15: KP oversampling with feature jitter (no exact duplicates)")
    print("  10.16: retry-on-low-AUC logic REMOVED (no selection bias)")
    print("  10.8:  PPO hyperparams from config (was hardcoded)")
    print("  9.4:   bridge uses VERIFIED AUC for gate (was trainer AUC)")
    print("  9.5:   bridge RAISES on validation failure in strict mode (no 0.35 AUC hole)")
    print("  9.7:   bridge checks GT/RL package version compatibility")
    print("  9.8:   bridge loads gt_checkpoint.pt (was save-only)")
    print("  9.11:  safety/market/unmet_need scores are per-property (no per-row noise)")
    print("  9.13:  pathway_score uses log1p(n)/log1p(max) (no saturation at 5+)")
    print("  9.14:  efficacy_score uses target diversity (was circular treats count)")
    print("  8.1:   trainer.train() alias for fit()")
    print("  8.2:   trainer.evaluate() no-arg path (sklearn-style)")
    print("  8.3:   trainer generator on correct device (was CPU-only)")
    print("  8.5:   trainer enforces drug-aware split")
    print("  8.6:   trainer auto-computes pos_weight from class balance")
    print("  8.11:  trainer ALWAYS restores best_state_dict (S-12 'use final' removed)")
    print("  8.14:  checkpoint saves full schema (graph_schema, version)")
    print("  8.21:  evaluate() returns probs/pred_binary/labels")
    print("  7.1:   nn.Embedding init std=0.02 (was 1.0, BERT/GPT standard)")
    print("  6.1:   predict_probability saves/restores training state (no eval() leak)")
    print("  5.3:   cross-edge-type normalization (1/sqrt(num_edge_types))")
    print("  5.5:   FFN dropout reduced from 2 to 1 (standard transformer)")
    print("  5.4:   self_loop_weight init 0.5 (was 0.1, equal standing)")
    print("  3.1:   finalize() emits ALL 14 edge types (no silent drop)")
    print("  3.2:   reverse-edge synthesis deduplicates")
    print("  3.3:   add_edge deduplicates self-loops and duplicate edges")
    print("  1.3:   LABEL_LEAKING_EDGES covers all 4 direct drug-disease relations")
    print("=" * 70)
