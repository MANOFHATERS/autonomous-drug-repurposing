...
class GraphTransformerTrainer:
    ...
    def load_checkpoint(self, path: str) -> None:
        ...
        # P3-042 / P3-D09 ROOT FIX: removed the duplicate
        # ``self.best_epoch = checkpoint.get("best_epoch", 0)`` restore
        # that was on the very next line (a no-op). The previous code
        # restored best_epoch TWICE — once with the BUG #21 comment,
        # once with the BUG #33 comment. Both read the same field; the
        # second silently overwrote the first. We merge the comments and
        # keep a single restore.
        #
        # V90 ROOT FIX (BUG #21 + BUG #33): restore self.best_epoch from
        # checkpoint. The previous code loaded every field EXCEPT
        # best_epoch, leaving it at its __init__ default of 0. After
        # reload, the user could not tell which epoch produced the best
        # model. The fix restores it from the checkpoint.
