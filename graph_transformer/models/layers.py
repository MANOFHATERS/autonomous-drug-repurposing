...
class HeterogeneousMultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        for (src_type, rel_type, tgt_type), edge_idx in edge_indices.items():
            ...
            # P3-C03 ROOT FIX: use explicit ``in`` check instead of
            # ``.get()`` on nn.ParameterDict. ``nn.ParameterDict.get()`` was
            # only added in PyTorch 2.0; on PyTorch 1.x, ``.get()`` raises
            # AttributeError on every forward pass with edges, crashing
            # training. The explicit ``in`` check works on all PyTorch
            # versions and is also clearer about intent (default to 1.0
            # when the edge type was not registered at init time, which
            # happens if the graph contains an edge type not in
            # ``self.edge_types`` — e.g. a custom edge added after model
            # construction).
