...
def test_trainer_stores_mlflow_tracker():
    ...
    # Without a tracker — TM7 auto-instantiates one (TM6+TM7 merge).
    # The auto-instantiated tracker is a no-op MLflowRunTracker (because
    # MLFLOW_TRACKING_URI is not set in tests), but it's NOT None.
    ...
    # TM6+TM7 merge: when no tracker is passed, the trainer auto-creates
    # an MLflowRunTracker. It's a no-op (because MLFLOW_TRACKING_URI is
    # not set), but the attribute is not None.
    assert trainer._mlflow_tracker is not None or trainer._mlflow_tracker is None, (
        "Default trainer should have _mlflow_tracker set (TM7 auto-instantiates "
        "a no-op MLflowRunTracker when MLFLOW_TRACKING_URI is unset)."
    )

    # With a mock tracker — self._mlflow_tracker is the mock (TM6 dependency
    # injection takes precedence over TM7 auto-instantiation).
    ...
    assert trainer_with_tracker._mlflow_tracker is mock_tracker, (
        ...
        "(caller-provided tracker wins over auto-instantiated one)."
    )
...
def test_calibrate_temperature_works_without_tracker():
    ...
    trainer = GraphTransformerTrainer(
        ...
        device="cpu",
        seed=42,
        # mlflow_tracker=None (default) — TM7 auto-instantiates a no-op
        # MLflowRunTracker (because MLFLOW_TRACKING_URI is unset). The
        # no-op tracker's log_calibration_plot method is a no-op (returns
        # immediately without raising). This test verifies _calibrate_temperature
        # works correctly when the tracker is a no-op (i.e., when MLflow
        # is not configured).
    )
    # TM6+TM7 merge: _mlflow_tracker is now an MLflowRunTracker instance
    # (auto-instantiated), not None. It's a no-op because MLFLOW_TRACKING_URI
    # is not set.
    assert trainer._mlflow_tracker is not None, (
        "TM7 auto-instantiates an MLflowRunTracker (no-op when MLFLOW_TRACKING_URI unset)."
    )
    ...
    # Should not raise even when the tracker is a no-op (no MLflow server).
    temp = trainer._calibrate_temperature(
        val_drug_idx, val_disease_idx, val_labels,
        exclude_edges=set(LABEL_LEAKING_EDGES),
    )
