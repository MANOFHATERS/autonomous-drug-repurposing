...
class GTRLBridge:
    ...
    def generate_rl_input(
        ...
        # P3-008 ROOT FIX (HIGH, numerical): with fp32 gnn_scores (from
        # torch.sigmoid on fp32 logits), the binary entropy computation
        # can produce a value SLIGHTLY larger than np.log(2) due to
        # floating-point precision (e.g., entropy=0.6931472... vs
        # log(2)=0.6931471...). This made confidence = 1.0 - 1.0000001 =
        # -1e-9 (slightly negative). The RL pipeline's validate_input_schema
        # then warned "Column 'confidence' has N values outside [0,1]" and
        # clipped them to 0.0 — spurious warnings on every run AND silent
        # clipping that masks real numerical instability if it ever occurs.
