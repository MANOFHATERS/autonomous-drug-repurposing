...
class GTRLBridge:
    ...
    def run_full_pipeline(
        ...
        # V92 ROOT FIX (BUG P3-004, CRITICAL): the line below previously
        # RE-ASSIGNED ``kp_recovery_threshold`` from
        # ``rl_config.min_kp_recovery_rate`` (default 0.2), silently
        # discarding the stricter ``max(rl_config_threshold, 0.5)`` value
        # computed at line 3241 (the V90 BUG #31 safety net). A coin-flip
        # model that recovered 1 of 2 test KPs by chance (50% recovery)
        # passed the gate, and broken models shipped to pharma partners.
        # The fix: REMOVE the reassignment. The ``kp_recovery_threshold``
        # computed at line 3241 (already ``max(rl_config_threshold, 0.5)``)
        # is the value used by the scientific_validation gate below.
