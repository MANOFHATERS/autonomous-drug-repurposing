...
describe("BE-022: GT checkpoint search includes repo-root parent dir", () => {
  ...
  test("checkpoint candidate dirs include parent output_v100 path", () => {
    ...
    // BE-022 fix: candidate dirs must include path.resolve(_REPO_ROOT, "output_v100")
    expect(src).toMatch(/path\.resolve\(_REPO_ROOT,\s*["']output_v100["']\)/);
    // And _REPO_ROOT must be derived from process.cwd() when running from frontend/
...
describe("BE-026: rl-ranker respects upstream.total (not upstream.count)", () => {
  test("source code uses upstream.total, not upstream.count, for total", () => {
    ...
    // The BE-026 fix replaces `total: upstream.count` with `total: upstreamTotal`
    // (which prefers upstream.total when present). Verify the fix is present.
    expect(src).toMatch(/upstreamTotal/);
    expect(src).toMatch(/typeof upstream\.total === "number"/);
    // The OLD buggy line must NOT appear in the proxy path.
    // (It may appear in comments explaining the bug.)
    ...
    // assignment (the bug). It may contain `upstreamTotal` which derives
    // from upstream.total.
