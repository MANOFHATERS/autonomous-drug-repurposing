...
def _is_withdrawn_drug(drug_name: str) -> bool:
    """P4-031 ROOT FIX: check if a drug name matches a withdrawn drug.
    ...
    Returns:
        True if the drug matches any withdrawn drug.
    """
    if not drug_name:
        return False
...
class RewardFunction:
    ...
    def compute(self, row: pd.Series) -> float:
        ...
        # P4-031 ROOT FIX: use substring + normalized matching instead of
        # exact match. The previous exact match missed salt forms (e.g.,
        # "thalidomide hydrochloride" does NOT match "thalidomide"),
        # brand-name suffixes, and stereoisomers — letting withdrawn
        # drugs slip through the safety gate. The fix uses a helper
        # function that checks both exact match AND substring containment
        # in both directions (covers salt forms and combination drugs).
        drug_name = str(row.get(DRUG_COL, "")).lower().strip()
        if _is_withdrawn_drug(drug_name):
            return -1.0
