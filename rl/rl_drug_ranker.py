...
DISEASE_NAMES: List[str] = [
    ...
    # P4-001 ROOT FIX: add the 4 validated-hypothesis diseases so the
    # data flywheel (DOCX §10) is reachable from the demo graph. These
    # are the diseases paired with the 4 validated drugs in
...
def save_results(
    ...
    P4-003 ROOT FIX (HIGH — Team Cosmic / Phase 4): REFUSES to write the
    CSV when ``metadata["_standalone_mode"]`` is True. The standalone
    mode (generate_fake_data) produces per-pair random features that
    DO NOT match the bridge's graph-derived features. An agent trained
    ...
    The fix blocks the CSV write at the source: if the metadata flags
    the run as standalone, save_results raises RuntimeError with a
    clear message directing the user to the bridge. The checkpoint
    save is ALSO blocked in train_agent (the _standalone_mode flag on
    the env). Together these two gates ensure NO standalone-trained
    artifact (CSV or checkpoint) can be persisted to disk.
    """
    import stat
    ...
    # P4-003 ROOT FIX: refuse to write the CSV if the run was in
    # standalone mode. The train_agent function already refuses to save
    # the checkpoint; this is the second gate that refuses to save the
    # CSV. Together they make it impossible to persist a standalone-
    # trained artifact to disk.
    if meta.get("_standalone_mode", False):
        _reason = meta.get("_standalone_mode_reason", "standalone mode (generate_fake_data)")
        raise RuntimeError(
            f"P4-003 ROOT FIX: REFUSING to write output CSV because the "
            f"run was in STANDALONE mode. Standalone mode produces "
            f"per-pair random features that DO NOT match the bridge's "
            f"graph-derived features — deploying this CSV on bridge "
            f"data would produce GARBAGE rankings. Reason: {_reason} "
