...
def bulk_upsert_dpi(
    ...
    *,
    ...
    # The second line was a NO-OP (replacing NaN with None in a column
    # that's already NaN does nothing). Worse, the DB UniqueConstraint
    # ``uq_dpi_drug_protein_source`` on (drug_id, protein_id, source,
    # source_id) uses NULLS DISTINCT semantics on PostgreSQL 15+ and
    # SQLite — so two rows with the SAME (drug_id, protein_id, source)
    # but NULL source_id are NOT considered duplicates by the DB. The
    # KG would silently accumulate duplicate DPI edges whenever a
    # source omits source_id (which is common for older ChEMBL records).
