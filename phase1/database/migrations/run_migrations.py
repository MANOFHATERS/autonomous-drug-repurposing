...
def _split_sql_statements(sql_content: str) -> list[str]:
    ...
    while i < n:
        ...
        if ch == ";":
            ...
            # v76 ROOT FIX (T-047): use the comprehensive
            # ``_is_transaction_control_statement`` filter instead of the
            # old bare BEGIN/COMMIT check. This catches ALL standard SQL
            # transaction-control forms (BEGIN TRANSACTION, START
            # TRANSACTION, COMMIT WORK, ROLLBACK, END, SAVEPOINT, etc.)
            # so none can leak through and interfere with the
            # ``engine.begin()`` transaction wrapper.
    ...
    stmt = "".join(current).strip()
    upper = stmt.upper().strip()
    # v76 T-047: comprehensive transaction-control filter for trailing buffer.
    if stmt and not _is_transaction_control_statement(upper):
        statements.append(stmt)
