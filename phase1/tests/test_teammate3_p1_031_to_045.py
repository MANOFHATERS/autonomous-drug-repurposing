...
def test_p1_033_validate_output_uses_explicit_compression_infer():
    """validate_output._validate_source must use a robust gzip-aware reader
    (either ``compression="infer"`` OR Teammate 2's ``_read_csv_robust`` /
    ``_robust_read_csv`` helper that sniffs gzip magic bytes).

    P1-033 + P1-016 combined: Teammate 2's P1-016 added magic-byte sniffing
    which is MORE robust than extension-based inference. We accept EITHER
    approach as long as .gz files are handled.
    """
    validate_output_path = _PHASE1_ROOT / "contracts" / "validate_output.py"
    content = validate_output_path.read_text()
    # Must use EITHER _read_csv_robust OR compression="infer".
    has_robust = "_read_csv_robust" in content
    has_infer = 'compression="infer"' in content
    assert has_robust or has_infer, (
        "validate_output.py must use either _read_csv_robust (P1-016) or "
        "compression='infer' (P1-033) to handle .gz files"
    )
    # Functional test: verify a .gz file can be read.
    import pandas as pd
    from contracts.validate_output import _validate_source
    from contracts.phase1_schema import PHASE1_OUTPUT_SCHEMA, get_all_aliases
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if gz_alias is None:
            print("PASS: P1-033 — robust reader present (no .gz alias to test)")
            return
        gz_path = tmpdir / gz_alias
        df = pd.DataFrame({"inchikey": ["ABCDEFGHijklMN0pQR"], "name": ["Test Drug"]})
    ...
    print(f"PASS: P1-033 — robust gzip-aware reader present ({'magic-byte sniffing' if has_robust else 'compression=infer'})")
