...
v89 FORENSIC ROOT FIX (BUG #8 P1 -- Sunday Morning Pile-Up):
  Originally moved from ``0 4 1 * *`` (1st of month) to ``0 4 15 * *``
  ...
  annotation below), which moved the schedule to weekly Friday at
  04:00 UTC. The weekly cadence is safer than a fixed day-of-month
  ...
  in the ``@dag(schedule=...)`` annotation below and is ``0 4 * * 5``
