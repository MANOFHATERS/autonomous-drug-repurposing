...
def run_entity_resolution() -> Dict[str, Any]:
    ...
    if string_aliases_df.empty:
        try:
            ...
            # This else: clause runs when the try block completed WITHOUT
            # raising any exception (i.e. no corrupt-file RuntimeError and
            # no other Exception). The previous code had this else: misplaced
            # INSIDE the except RuntimeError: block (right after `raise`),
            # which is invalid Python syntax (else: cannot follow raise
            # inside an except clause). It was moved here (at the same
            # indent level as the except clauses) so it correctly runs only
            # when the try block succeeded but no human aliases file was
            # found.
            if _string_raw_dir.exists():
