...
# v121 ROOT FIX (Teammate 5 — hostile-auditor pass):
# config_schema.py line 28-32 has a comment claiming:
#   "CORE_EDGE_TYPES (below) MUST only reference node types from this
#    list (or from DRKG_NODE_TYPES for legacy DRKG edges). The
#    kg_builder._validate_core_edge_types invariant check enforces
#    this at import time."
# But NO SUCH INVARIANT CHECK EXISTED — the comment was a lie (exactly
# the "aspirational claim" pattern the user warned about). P2-004 was
# fixed by adding "Drug" to CORE_NODE_TYPES, but the SAME class of
# invariant violation existed for "Anatomy" (referenced by
# ("Gene", "expressed_in", "Anatomy") and
# ("Protein", "expressed_in", "Anatomy") in CORE_EDGE_TYPES).
#
# Anatomy happens to be in DRKG_NODE_TYPES, so it would be accepted by
# the (non-existent) check — but the check did not exist at all, so a
# FUTURE addition of, say, ("Compound", "targets", "Receptor") would
# silently pass with "Receptor" neither in CORE_NODE_TYPES nor
# DRKG_NODE_TYPES, producing a Cypher query that references an
# unindexed :Receptor label and silently dead-lettering every edge.
#
# ROOT FIX: add the actual invariant check that the comment promised.
# It validates that every src and dst label in CORE_EDGE_TYPES is in
# CORE_NODE_TYPES ∪ DRKG_NODE_TYPES. This is a STRUCTURAL invariant —
# violating it produces silent data loss (load_edges_batch dead-letters
# any edge type not in CORE_EDGE_TYPES_SET, but a node-type typo like
# "Compound " → " Compound" would also fail the whitelist lookup
# because the label has whitespace).
def _validate_core_edge_types_node_refs() -> None:
    """Validate that every CORE_EDGE_TYPES entry references known node types.

    Each (src, rel, dst) triple's src and dst MUST be in
    CORE_NODE_TYPES ∪ DRKG_NODE_TYPES. Otherwise:
      - load_edges_batch's whitelist check passes (the triple is in
        CORE_EDGE_TYPES_SET), but
      - The Cypher generated by kg_builder references a label that has
        no Neo4j index/constraint, producing a full-graph scan on
        every load (silent performance regression), AND
      - The Phase 2→3 adapter has no mapping for the unknown node
        type (silent data loss at the Phase 3 boundary).

    This invariant was claimed to exist by a comment in
    ``config_schema.py`` (line 28-32) but was never actually
    implemented — a classic "aspirational comment" pattern. This
    function implements it for real.
    """
    try:
        ...
        # Config not available (direct-script mode) — skip.
        return
    ...
    for src, rel, dst in CORE_EDGE_TYPES:
        if src not in allowed:
            violations.append((src, rel, dst, "src"))
        if dst not in allowed:
            violations.append((src, rel, dst, "dst"))
    if violations:
        details = "; ".join(
            f"({s!r}, {r!r}, {d!r}) — {which} not in CORE_NODE_TYPES "
            f"({sorted(CORE_NODE_TYPES)}) nor DRKG_NODE_TYPES "
            f"({sorted(DRKG_NODE_TYPES)})"
            for s, r, d, which in violations
        )
        raise RuntimeError(
            "P2-004-CLASS invariant violated: CORE_EDGE_TYPES contains "
            f"triples referencing unknown node types: {details}. Every "
            "src/dst MUST be in CORE_NODE_TYPES ∪ DRKG_NODE_TYPES, "
            "otherwise the Cypher generated by kg_builder references "
            "an unindexed label (silent perf regression) and the "
            "Phase 2→3 adapter has no mapping (silent data loss). "
            "Either add the node type to CORE_NODE_TYPES or remove "
            "the edge from CORE_EDGE_TYPES."
        )
...
if not _import_time_skip:
    ...
    # v121: also run the new node-ref invariant check.
    try:
        _validate_core_edge_types_node_refs()
    except RuntimeError as _e:
        logging.getLogger(__name__).critical(
            "P2-004-CLASS invariant violated at import time: %s. "
            "Module is importable for debugging, but the runtime "
            "check in DrugOSGraphBuilder.__init__ will raise.",
            _e,
        )
