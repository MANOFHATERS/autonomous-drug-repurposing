...
def bridge_to_pyg_maps(
    ...
    #
    # P2-005 FORENSIC ROOT FIX (Team 4 -- biologic drugs appear as 2 PyG
    # Compound nodes). The previous code indexed Compound nodes by
    # ``n["id"]`` alone. But for biologic drugs (insulin, mAbs, vaccines --
    # ~30% of modern FDA approvals) there is no InChIKey, so
    # ``canonical_id = drugbank_id`` (e.g. "DB00071"). ChEMBL and PubChem
    # emit the SAME compound with an inchikey canonical_id (e.g.
    # "RZVAJINKQORUOD-..."). The biologic therefore appeared as TWO
    # entries: "DB00071" and the inchikey -- they did NOT MERGE. Drug->Protein
    # edges from DrugBank attached to "DB00071"; Drug->Protein edges from
    # ChEMBL attached to the inchikey node. The GNN learned two separate
    # embeddings for the same drug -- wasting capacity and breaking
    # multi-hop reasoning.
    #
    # ROOT FIX: for Compound nodes, consolidate via the
    # ``compound_id_aliases`` field (which kg_builder's MERGE already
    # uses, and which phase1_bridge now populates for ALL Compound nodes
    # -- see the v78 FORENSIC ROOT FIX in ``_load_drugbank_drugs``). When
    # a new Compound node is encountered, check if ANY of its
    # ``compound_id_aliases`` already exists in the map -- if so, treat
    # them as the same node (reuse the existing index). The FIRST
    # canonical ID encountered wins; subsequent aliases map to it. This
    # mirrors kg_builder's MERGE Cypher semantics.
    entity_maps: Dict[str, Dict[str, int]] = {}
    # P2-005: alias -> canonical_id map (Compound only). Tracks which
    # aliases have been seen so a later node with the same alias merges
    # into the FIRST canonical ID.
    _compound_alias_to_canonical: Dict[str, str] = {}
    for load in builder.node_loads:
        ...
        for i, n in enumerate(load["nodes"]):
            nid = n["id"]
            if label == "Compound":
                # P2-005: check if any alias of this node already maps to
                # an existing canonical ID. If so, merge into the existing
                # node (don't create a new entry).
                aliases = n.get("compound_id_aliases") or []
                existing_canonical = None
                for alias in aliases:
                    if alias and alias in _compound_alias_to_canonical:
                        existing_canonical = _compound_alias_to_canonical[alias]
                        break
                if existing_canonical is not None and existing_canonical in entity_maps[label]:
                    # Merge: map this node's nid AND all its aliases to
                    # the existing canonical ID's index. Don't create a
                    # new entry.
                    entity_maps[label][nid] = entity_maps[label][existing_canonical]
                    for alias in aliases:
                        if alias:
                            _compound_alias_to_canonical[alias] = existing_canonical
                    continue
                # New canonical Compound node -- register it and all its
                # aliases so future nodes with the same alias merge here.
                if nid not in entity_maps[label]:
                    entity_maps[label][nid] = len(entity_maps[label])
                _compound_alias_to_canonical[nid] = nid
                for alias in aliases:
                    if alias and alias not in _compound_alias_to_canonical:
                        _compound_alias_to_canonical[alias] = nid
            else:
                if nid not in entity_maps[label]:
                    entity_maps[label][nid] = len(entity_maps[label])
