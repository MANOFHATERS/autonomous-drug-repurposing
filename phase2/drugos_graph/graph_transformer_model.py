...
# P2-065 ROOT FIX: sentinel module that occupies the same ModuleDict
# slot as a real ``nn.Embedding`` but RAISES a clear RuntimeError when
# ACCESSED. This replaces the previous ``nn.Embedding(0, d)`` pattern
# which silently crashed with ``IndexError: index out of range in self``
# at forward() time when the caller forgot to call
# ``resize_node_embeddings``.
#
# Design rationale:
#   - Subclass ``nn.Module`` (not ``nn.Embedding``) so it has NO weight
#     tensor — accessing ``.weight`` raises immediately. This is the
#     key safety property: the previous ``nn.Embedding(0, d)`` HAD a
#     weight tensor (shape ``(0, d)``), so the model saved/loaded
#     cleanly, and the crash only fired at forward() time when an
#     out-of-range index was looked up. The sentinel makes the crash
#     fire at ACCESS time (``.weight`` or ``__call__``) with a clear
#     message naming the node type.
#   - Store ``node_type`` and ``embedding_dim`` as attributes so the
#     error message can name the specific node type and tell the
#     operator exactly which ``resize_node_embeddings`` call to make.
#   - The sentinel is a NO-OP in the state_dict (it has no parameters),
#     so loading a checkpoint that was saved with ``nn.Embedding(0, d)``
#     into a model with the sentinel works cleanly (the 0-row weight is
#     silently dropped). This preserves backward checkpoint compat.
class _PendingEmbedding(nn.Module):
    """Sentinel for a feature-less node type whose embedding table has
    not yet been resized.

    P2-065 root fix: replaces ``nn.Embedding(0, d)`` so that a forgotten
    ``resize_node_embeddings`` call produces a CLEAR RuntimeError naming
    the offending node type, instead of a cryptic
    ``IndexError: index out of range in self`` from deep inside PyTorch's
    embedding lookup kernel.
    """

    def __init__(self, embedding_dim: int, node_type: str = "<unknown>") -> None:
        super().__init__()
        self.embedding_dim = int(embedding_dim)
        self.node_type = str(node_type)
        # No parameters registered — the sentinel is a pure placeholder.
        # Loading a checkpoint that contained a real ``nn.Embedding(0, d)``
        # weight for this node type will silently drop the 0-row weight
        # (no shape mismatch, no error) because this module has no
        # ``weight`` attribute to load into.

    def _raise(self) -> "torch.Tensor":  # never returns; for type-checkers
        raise RuntimeError(
            f"P2-065: node type {self.node_type!r} has no input features "
            f"AND its embedding table has not been resized. The model was "
            f"constructed with node_feature_dims that did NOT include "
            f"{self.node_type!r}, so the table started at size 0. The "
            f"caller MUST call "
            f"model.resize_node_embeddings({{{self.node_type!r}: N}}) "
            f"with the actual node count before calling forward() or "
            f"encode(). (Previously this crashed with a cryptic "
            f"'IndexError: index out of range in self' from PyTorch's "
            f"embedding lookup kernel — the error message gave no hint "
            f"which node type was empty.)"
        )

    @property
    def weight(self) -> "torch.Tensor":  # type: ignore[override]
        # Any access to ``.weight`` (e.g. ``model.node_embedding_tables[
        # nt].weight``) raises immediately. This is the primary access
        # path used by ``encode()`` to fetch features for feature-less
        # node types.
        return self._raise()  # type: ignore[return-value]

    def forward(self, indices: "torch.Tensor") -> "torch.Tensor":  # type: ignore[override]
        # Direct call (e.g. ``table(torch.tensor([0, 1, 2]))``) also
        # raises. This catches the case where a caller tries to look up
        # specific node indices before resize.
        return self._raise()  # type: ignore[return-value]

    def __repr__(self) -> str:
        return (
            f"_PendingEmbedding(node_type={self.node_type!r}, "
            f"embedding_dim={self.embedding_dim}) — call "
            f"resize_node_embeddings() before forward()."
        )
...
class GraphTransformerModel(nn.Module):
    ...
    def __init__(
        ...
        # P2-065 ROOT FIX: track node types that have NO features and
        # NO embedding size at construction time. The previous code
        # silently allocated ``nn.Embedding(0, d)`` for these types —
        # a zero-size embedding that crashes with a cryptic
        # ``IndexError: index out of range in self`` when the caller
        # forgets to call ``resize_node_embeddings`` before ``forward()``.
        # The error message gave NO indication which node type's table
        # was empty, so operators had to grep the codebase to find the
        # missing resize call.
        #
        # Root fix (3 layers):
        #   (1) At construction: log a WARNING listing every node type
        #       that has no features. The operator sees the warning in
        #       the run log and knows they must call
        #       ``resize_node_embeddings`` before ``forward()``.
        #   (2) Replace the silent ``nn.Embedding(0, d)`` with a
        #       ``_PendingEmbedding`` sentinel that RAISES a clear
        #       RuntimeError when ACCESSED. The sentinel occupies the
        #       same ModuleDict slot so the model state_dict layout is
        #       unchanged (no checkpoint incompatibility), but any
        #       attempt to READ from the table before resize produces
        #       an actionable error message naming the node type.
        #   (3) Add a ``_validate_embedding_tables()`` method that
        #       ``encode()`` and ``forward()`` call at entry. This
        #       catches the "forgot to resize" case BEFORE the cryptic
        #       IndexError fires deep in the HGT layer stack.
        _pending_node_types: List[str] = []
        for nt in self.node_types:
            ...
            if in_dim and in_dim > 0:
                ...
                # P2-065: use the sentinel so a forgotten resize
                # produces a CLEAR error instead of a cryptic
                # ``IndexError: index out of range in self``.
                self.node_embedding_tables[nt] = _PendingEmbedding(
                    d, node_type=nt,
                )
                _pending_node_types.append(nt)
        if _pending_node_types:
            logger.warning(
                "P2-065 ROOT FIX: GraphTransformerModel constructed with "
                "%d node type(s) that have NO input features and NO "
                "embedding size: %s. The caller MUST call "
                "resize_node_embeddings({nt: N for nt in these types}) "
                "before forward()/encode(). Until resize is called, any "
                "attempt to read embeddings for these node types will "
                "raise a clear RuntimeError naming the node type. "
                "(Previously the model silently allocated "
                "nn.Embedding(0, d) and crashed with a cryptic "
                "'index out of range in self' on first forward.)",
                len(_pending_node_types), _pending_node_types,
            )
    ...
    def resize_node_embeddings(
        ...
        for nt, n in node_counts.items():
            ...
            if nt in self.node_embedding_tables:
                ...
                # P2-065: if the old table is a _PendingEmbedding
                # sentinel (constructed with no features and not yet
                # resized), there is NO weight to preserve — skip the
                # copy. Accessing ``old.weight`` on a _PendingEmbedding
                # would raise the P2-065 RuntimeError, which is the
                # WRONG behavior here (resize is the LEGITIMATE way to
                # resolve the pending state). Use isinstance() to
                # detect the sentinel and skip the weight copy.
                if not isinstance(old, _PendingEmbedding):
    ...
    def _validate_embedding_tables(self) -> None:
        """P2-065 ROOT FIX: verify that no node_embedding_table is still
        in the ``_PendingEmbedding`` sentinel state.

        Called from ``encode()`` and ``forward()`` at entry so a
        forgotten ``resize_node_embeddings`` call fails FAST with a
        clear, actionable error message — instead of a cryptic
        ``IndexError: index out of range in self`` from deep inside
        PyTorch's embedding lookup kernel.

        The check is O(num_node_types) — negligible vs. the O(E) HGT
        forward pass — so we run it on EVERY call. This is intentional:
        a model that was correctly resized at construction time but
        later re-loaded with a stale state_dict (where the resize was
        lost) should fail on the first forward, not silently produce
        NaN gradients.
        """
        pending = [
            nt for nt, tbl in self.node_embedding_tables.items()
            if isinstance(tbl, _PendingEmbedding)
        ]
        if pending:
            raise RuntimeError(
                f"P2-065: cannot run encode()/forward() — the following "
                f"node type(s) still have _PendingEmbedding sentinels "
                f"(resize_node_embeddings was never called for them): "
                f"{pending}. Call "
                f"model.resize_node_embeddings({{nt: N for nt in {pending}}}) "
                f"with the actual node counts before calling "
                f"encode()/forward(). (P2-065 root fix: previously this "
                f"crashed with a cryptic 'IndexError: index out of range "
                f"in self' from PyTorch's embedding lookup kernel, with "
                f"no indication which node type was empty.)"
            )
    ...
    def encode(
        ...
        """
        # P2-065 ROOT FIX: fail fast if any embedding table is still a
        # _PendingEmbedding sentinel. The previous code with
        # ``nn.Embedding(0, d)`` would crash here with a cryptic
        # ``IndexError: index out of range in self`` deep in the HGT
        # forward pass — no indication which node type was empty.
        self._validate_embedding_tables()
