...
def step11b_train_graph_transformer(
    ...
    def _make_negatives(positive_indices, rng=None):
        ...
        for i in positive_indices:
            ...
            if not found:
                n_skipped_no_neg += 1
                # P2-002 ROOT FIX: keep `negs` aligned 1:1 with
                # positive_indices. Previously, when no valid negative
                # existed for a positive (saturated coverage), the
                # positive was silently DROPPED — so len(negs) <
                # len(positive_indices). Downstream code then sliced
                # train_negatives_all[start:end] and ASSUMED alignment
                # with train_idx[start:end], silently pairing each
                # positive with the WRONG negative. The fix: pad with
                # (0, 0). These (h=0, t=0) tuples will be filtered out
                # by the NaN-filter (decoder key check) if h=0 or t=0
                # is not a valid Compound/Disease, OR will produce
