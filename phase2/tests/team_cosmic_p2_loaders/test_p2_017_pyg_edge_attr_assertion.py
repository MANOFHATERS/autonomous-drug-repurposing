...
Severity: HIGH  |  Category: Wrong  |  File: phase2/drugos_graph/pyg_builder.py  |  Line: 1797-1843
...
def test_p2_017_pyg_edge_attr_assertion():
    """HIGH: torch.flip call sites must assert edge_attr is None."""
    from drugos_graph.pyg_builder import PyGBuilder
    src = inspect.getsource(PyGBuilder)
    assert "torch.flip" in src
    assert "P2-017" in src
    assert src.count("assert") >= 2, f"only {src.count('assert')} asserts"
    assert "edge_attr" in src and "is None" in src
