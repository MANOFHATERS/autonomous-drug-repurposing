...
def test_p4_031_withdrawn_drug_matches_salt_forms_and_suffixes():
    ...
    from rl.rl_drug_ranker import _is_withdrawn_drug
    # Exact match
    assert _is_withdrawn_drug("thalidomide")
    assert _is_withdrawn_drug("vioxx")
    # Salt forms (the original bug)
    assert _is_withdrawn_drug("thalidomide hydrochloride"), (
        "P4-031 FAIL: 'thalidomide hydrochloride' should match 'thalidomide'"
    )
    assert _is_withdrawn_drug("thalidomide hcl"), (
        "P4-031 FAIL: 'thalidomide hcl' should match 'thalidomide'"
    )
    # Suffix forms (brand name with dose)
    assert _is_withdrawn_drug("Thalidomide-50"), (
        "P4-031 FAIL: 'Thalidomide-50' should match 'thalidomide'"
    )
    assert _is_withdrawn_drug("Vioxx 50mg"), (
        "P4-031 FAIL: 'Vioxx 50mg' should match 'vioxx'"
    )
    # Combination drugs
    assert _is_withdrawn_drug("thalidomide/aspirin combo"), (
        "P4-031 FAIL: combination drug with thalidomide should match"
    )
    # Negative cases
