...
def test_P3_004_gt_rl_bridge_no_threshold_relowering():
    ...
    # The re-lowering line must be GONE. Look for the exact pattern that
    # would re-assign kp_recovery_threshold to the rl_config default.
    import re
    relowering = re.findall(
        r"kp_recovery_threshold\s*=\s*float\(getattr\(rl_config,[^)]+\)\)",
        src,
    )
    assert len(relowering) == 0, (
        f"P3-004 REGRESSION: gt_rl_bridge.py re-lowers kp_recovery_threshold: {relowering}"
    )
...
def test_P3_005_pathway_diff_runs_unconditionally():
    ...
    # Find the function: _compute_supplementary_features or wherever the
    # unmet_need_for_disease closure lives. Look for the S-F1 fix marker.
    assert "v100 ROOT FIX (P3-005)" in src, (
        "P3-005 INCOMPLETE: P3-005 fix marker not found"
    )
...
def test_P1_001_chembl_no_double_read():
    ...
    # AFTER the try/except) must be gone. Look for the marker.
    assert "v100 ROOT FIX (P1-001)" in src, (
    ...
    # Look for the specific dead pattern.
    assert '_compression = "gzip" if raw_path.suffix == ".gz" else None\n        drugs_df = pd.read_csv(\n            raw_path,\n            compression=_compression,' not in src, (
        "P1-001 REGRESSION: chembl_pipeline.py still has the double-read"
    )
