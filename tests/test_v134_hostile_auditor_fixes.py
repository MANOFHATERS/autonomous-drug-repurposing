...
def test_fix1_backend_predict_wired_to_gt_service():
    ...
    assert "gt_url.rstrip('/')}/predict" in source or "f\"{gt_url.rstrip('/')}/predict" in source, \
        "BUG: backend /predict does not construct GT_SERVICE_URL/predict URL"
    # Must raise 503 when GT_SERVICE_URL is not configured (not return fake data)
    assert "503" in source and "GT_SERVICE_URL not configured" in source, \
        "BUG: backend /predict does not raise 503 when GT_SERVICE_URL is missing"
...
def test_fix2_backend_top_k_wired_to_rl_service():
    ...
    source = main_py.read_text(encoding="utf-8")
    # The hardcoded placeholder must be GONE
    assert "candidates=[]," not in source or "candidates=list(rl_payload" in source, \
        "BUG: backend /top-k still returns hardcoded candidates=[]"
    ...
    assert "rl_url.rstrip('/')}/rank" in source or "f\"{rl_url.rstrip('/')}/rank" in source, \
...
def test_fix4_build_demo_graph_no_self_assignment():
    ...
    # Verify the body DOES reference builder.validated_pairs in executable code
    assert "builder.validated_pairs" in executable_body, \
        "BUG: build_demo_graph does not write to builder.validated_pairs (the local instance)"
