...
def test_backend_ready_endpoint_probes_rl_service():
    ...
    The /ready check returns 200 (status='ok') when ALL downstream
    dependencies are reachable; 503 (status='degraded') when any fail.
    This test verifies the RL service probe works.
    """
    from fastapi.testclient import TestClient
    from backend.api.main import app
    ...
    assert "rl_service" in data["checks"]
    # RL service probe should succeed (we mocked it as 200).
    assert data["checks"]["rl_service"] is True
