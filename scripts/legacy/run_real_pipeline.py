...
def main() -> int:  # R-027: was -> None with sys.exit()
    ...
    # of run_4phase.py with different default hyperparameters (gt_epochs=500,
    # rl_timesteps=50000 vs run_4phase.py's 80/5000) and a different adapter
    # path (from_phase1_staged_data vs adapt_phase2_to_phase3). Per the
    # ORCH-003 root fix, the platform consolidates on run_4phase.py as the
    # SINGLE canonical 4-phase runner. The higher hyperparameters from this
    # runner can be reproduced via `python run_4phase.py --gt-epochs 500
    # --rl-timesteps 50000`. This file is preserved for backward
    # compatibility but emits a stderr deprecation warning on every
    # invocation. To silence, switch to run_4phase.py.
    import sys as _sys
    _sys.stderr.write(
        "=" * 72 + "\n"
        "ORCH-003 DEPRECATION NOTICE: run_real_pipeline.py is deprecated.\n"
        ...
        "  Action: replace `python run_real_pipeline.py` with\n"
        "    `python run_4phase.py --gt-epochs 500 --rl-timesteps 50000`\n"
        "    to reproduce this runner's higher hyperparameters.\n"
    ...
    try:
        ...
        # run_real_pipeline.py consistent with it.
